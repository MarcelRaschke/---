.PHONY: help setup start stop restart logs status health update clean prod monitoring doctor backup
...
prod: ## Start with production overlay (nginx + TLS)
	docker compose -f compose/docker-compose.yml -f compose/docker-compose.prod.yml up -d

monitoring: ## Start with monitoring overlay (Prometheus + Grafana)
